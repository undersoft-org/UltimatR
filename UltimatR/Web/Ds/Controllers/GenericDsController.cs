...
namespace UltimatR
{
    ...
    public abstract class GenericDsController<TKey, TStore, TEntity> : ODataController where TEntity : Entity where TStore : IDataStore
    {
        protected readonly Func<TKey, Expression<Func<TEntity, bool>>> _keymatcher;        
        protected readonly IUltimatr _ultimatr;

        protected GenericDsController() { }
        protected GenericDsController(IUltimatr ultimatr) : this(ultimatr, k => e => k.Equals(e.Id))
        {                          
        }
        protected GenericDsController(IUltimatr ultimatr, Func<TKey, Expression<Func<TEntity, bool>>> keymatcher)
        {
