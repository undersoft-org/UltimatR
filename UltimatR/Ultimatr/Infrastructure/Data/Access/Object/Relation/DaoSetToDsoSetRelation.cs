...
namespace UltimatR
{
    public class DaoSetToDsoSetRelation<TLeft, TRight> where TLeft : Entity where TRight : Entity
    {
        ...
        public DaoSetToDsoSetRelation(ModelBuilder modelBuilder, ExpandSite expandSite = ExpandSite.OnRight, string dbSchema = null)
           : this(modelBuilder, null, null, null, null, expandSite, dbSchema, dbSchema) { }

        public DaoSetToDsoSetRelation(ModelBuilder modelBuilder, string leftName, string rightName, ExpandSite expandSite = ExpandSite.OnRight, string dbSchema = null)
            : this(modelBuilder, leftName, leftName, rightName, rightName, expandSite, dbSchema, dbSchema) { }

        public DaoSetToDsoSetRelation(ModelBuilder modelBuilder,
                                       string leftName,
        ...
        public ModelBuilder Configure()
        {
            ...
            _relationBuilder.HasOne(a => a.RightEntity)
                            .WithMany(RELATION_TABLE_NAME)
                            .HasForeignKey(a => a.RightEntityId);

            _relationBuilder.HasOne(a => a.LeftEntity)
                            .WithMany(RELATION_TABLE_NAME)
                            .HasForeignKey(a => a.LeftEntityId);

            _firstBuilder.HasMany<DaoRelation<TLeft, TRight>>(RELATION_TABLE_NAME)
                         .WithOne(p => p.LeftEntity)
                         .HasForeignKey(k => k.LeftEntityId);

            _secondBuilder.HasMany<DaoRelation<TLeft, TRight>>(RELATION_TABLE_NAME)
