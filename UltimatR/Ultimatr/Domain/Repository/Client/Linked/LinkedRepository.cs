...
namespace UltimatR
{
    ...
    {
        public LinkedRepository(IRepositoryContextPool<DsContext<TStore>> pool, IEntityCache<TStore, TEntity> cache) : base(
            pool.ContextPool)
        {
            mapper = cache.Mapper;
            this.cache = cache;
        ...
        where TEntity : class, IIdentifiable
    {
        ...
        public LinkedRepository(DsContext context) : base(context)
        {
            if(dsContext != null)
            {
                dsQuery = dsContext.CreateQuery<TEntity>(typeof(TEntity).Name, false);
