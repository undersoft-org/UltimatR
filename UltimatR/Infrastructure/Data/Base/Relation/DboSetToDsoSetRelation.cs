...
namespace UltimatR
{
    public class DboSetToDsoSetRelation<TLeft, TRight> where TLeft : Entity where TRight : Entity
    {
        ...
        public DboSetToDsoSetRelation(ModelBuilder modelBuilder, ExpandSite expandSite = ExpandSite.OnRight, string dbSchema = null)
           : this(modelBuilder, null, null, null, null, expandSite, dbSchema, dbSchema) { }

        public DboSetToDsoSetRelation(ModelBuilder modelBuilder, string leftName, string rightName, ExpandSite expandSite = ExpandSite.OnRight, string dbSchema = null)
            : this(modelBuilder, leftName, leftName, rightName, rightName, expandSite, dbSchema, dbSchema) { }

        public DboSetToDsoSetRelation(ModelBuilder modelBuilder,
                                       string leftName,
        ...
        public ModelBuilder Configure()
        {
            ...
            _firstBuilder.HasMany<DboRelation<TLeft, TRight>>(RELATION_TABLE_NAME)
                         .WithOne(p => p.LeftEntity)
                         .HasForeignKey(k => k.LeftEntityId);

            _secondBuilder.HasMany<DboRelation<TLeft, TRight>>(RELATION_TABLE_NAME)
