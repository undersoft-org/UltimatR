//-----------------------------------------------------------------------
// <copyright file="BoardBase.cs" company="Undersoft">
//     Author: Dariusz Hanc
...
namespace System.Series
{
    ...
    public abstract class BoardBase<V> : DeckBase<V>
    {
        ...
        protected BoardBase(IEnumerable<IUnique<V>> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(
            capacity,
            bits)
        {
            if(collection != null)
        ...
        protected BoardBase(IEnumerable<V> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(
            capacity,
            bits)
        {
            if(collection != null)
        ...
        protected BoardBase(IList<IUnique<V>> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(
        ...
        protected BoardBase(IList<V> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(
            (capacity > collection.Count) ? capacity : collection.Count,
            bits)
        {
            foreach(V c in collection)
